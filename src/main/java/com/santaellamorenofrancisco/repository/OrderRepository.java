...
public interface OrderRepository extends JpaRepository<Order, Long> {
	...
	List<Product> getProductsInShoppingCart(@Param("client_id") Long client_id);

	@Query(nativeQuery = true, value = "INSERT INTO _order(id, ispaid, num_order, order_date, client_id, product_id) VALUES (?1,?2,?3,?4,?5,?6")
	Boolean createOrder(@Param("id") Long id, @Param("ispaid")Boolean ispaid,@Param("num_order")Long num_order,@Param("order_date")LocalDateTime order_date,@Param("client_id") Long client_id,@Param("product_id") Long product_id);
}
